---
title: "Modelling_effects_of_SMDW_use_on_child_diarrhea"
format: html
editor: visual
---
...
data <- data %>% 
    mutate(Accessible.On.Premises = ifelse(
              WS4 == 0 | #WS4 = minutes taken to to collect water
             WS3 == 1 | WS3 == 2 | #WS3: 1 = water source is in dwelling, 2 = in yard/ on plot
                WS1 == 2 | WS2 == 2, #WS1, WS2: 2 = water is piped to premises
                          "YES", "NO"))
...
data<- data %>%
  mutate(Improved.Water.Source = ifelse(WS1 == 1| #1 = Improved but not explicitely on plot
                                  WS1 == 2| #2= Piped to dwelling, compound or yard (= to premises)
                                  WS1 == 3, #3= Bottled or packaged water
                                  "YES", "NO"))
...
data$Water.Source[data$Water.Source=="1"] <- "Other improved sources"
data$Water.Source[data$Water.Source=="3"] <- "Bottled or packages"
data$Water.Source[data$Water.Source=="2"] <- "Piped to premises"
...
#Total number of children: 527 402
data<-subset(data, WQ27 != -99) #78299
data<-subset(data, WQ26 != -99) #78051
data<-subset(data, WS7 != -99) #77873

levels(data$Accessible.On.Premises) #No NAs #77873
levels(data$Improved.Water.Source) #No NAs #77873

# Remove observations without diarrheal information from the original data frame

data <-subset(data, CA1 != "NA") #42815
data<-subset(data, CA1 != -99) #41683
...
```{r}
#| echo: false
#| Remove duplicate children from the original data frame 
#| Comment: Skip this step for sensitivity analysis
...
data.model1_precipitation <- data.model1_precipitation %>% 
    mutate(AtLeastBasic = ifelse(WS1  == 2 | #2= Piped to dwelling, compound or yard
              (WS1 == 1 & Less30MinRoundTrip == "YES") | #1= Other improved (not piped to premises or bottled/packaged)
              (WS1 == 3 & Less30MinRoundTrip == "YES"), #3= bottled or packaged water
                          "YES", "NO"))
...
data.model1_precipitation <- data.model1_precipitation %>% 
     mutate(SMDW = ifelse(WQ27 == 0 &    #Source water has no E.coli
                ...
                Drinking.Water.Sufficient == "YES",   #Available when needed = No insufficiency reported or known
                          "YES", "NO"))
...
```{r}
# creating data frame for plot with estimates and confidence intervals
...
#### 3.1.1 Sub-component model (log binomial regression)
```{r}
...
### 5.2 Running models and calculating confidence intervals
...
### 6.2 Running models and calculating confidence intervals
...
tetrachoric(table(data.model1_precipitation$Fecal.Contamination.POU, data.model1_precipitation$Fecal.Contamination.POC)) 
# tetrachoric correlation Improved water source vs E.coli at POC
tetrachoric(table(data.model1_precipitation$Improved.Water.Source, data.model1_precipitation$Fecal.Contamination.POC))
# tetrachoric correlation Improved water source vs E.coli at POU
tetrachoric(table(data.model1_precipitation$Improved.Water.Source, data.model1_precipitation$Fecal.Contamination.POU))
# tetrachoric correlation Accessibility vs E.coli at POU
tetrachoric(table(data.model1_precipitation$Accessible.On.Premises, data.model1_precipitation$Fecal.Contamination.POU))
# tetrachoric correlation Accessiblilty vs E.coli at POC
tetrachoric(table(data.model1_precipitation$Accessible.On.Premises, data.model1_precipitation$Fecal.Contamination.POC))
# tetrachoric correlation sufficiency vs E.coli at POC
tetrachoric(table(data.model1_precipitation$Drinking.Water.Sufficient, data.model1_precipitation$Fecal.Contamination.POC))
# tetrachoric correlation sufficiency vs E.coli at POU
tetrachoric(table(data.model1_precipitation$Drinking.Water.Sufficient, data.model1_precipitation$Fecal.Contamination.POU))

# tetrachoric correlation sufficiency vs improved source
tetrachoric(table(data.model1_precipitation$Drinking.Water.Sufficient, data.model1_precipitation$Improved.Water.Source))
# tetrachoric correlation sufficiency vs accessibility
tetrachoric(table(data.model1_precipitation$Drinking.Water.Sufficient, data.model1_precipitation$Accessible.On.Premises))
# tetrachoric correlation accessible vs improved source
tetrachoric(table(data.model1_precipitation$Accessible.On.Premises, data.model1_precipitation$Improved.Water.Source))
