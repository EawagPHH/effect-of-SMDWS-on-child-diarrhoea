...
setMainSourceLabels_otherCategories <- function(df,WS1) {
  #Unimproved
  df$WS1[df$WS1 == 32|df$WS1 == 42|df$WS1 == 43|df$WS1 == 81|df$WS1 == 96|
           df$WS1 == 99] <- 0
  
  #improved but not piped
  df$WS1[df$WS1 == 21|df$WS1 == 22|df$WS1 == 23|df$WS1 == 31
         |df$WS1 == 41|df$WS1 == 51|df$WS1 == 61|df$WS1 == 62|df$WS1 == 71|df$WS1 == 72 
         |df$WS1 == 93] <- 1
  
  #Piped
  df$WS1[df$WS1 == 13|df$WS1 == 14|df$WS1 == 11|df$WS1 == 12]<- 2
...
relabelingSurveyQuestionResponses <- function(df,WS1,WS2,WS3,WS4,WS7,WS8,WQ27){
  df <- setMainSourceLabels(df,WS1)
  df <- setSecondarySourceLabels(df,WS2)
  df <- setWS3_WslocationLabels(df,WS3)
  df <- setWS4timeLabels(df,WS4)
  df <- setWS7WaterInsufficiencyLabels(df,WS7)
  df <- setWS8WhyInsufficientLabels(df,WS8)
  df <- setWQ27SourceWaterQuality(df,WQ27)
  df <- setWRiskBIN(df, WQ27)
  return(df)
}
...
labelingSurveyVariableResponses <- function(df){
  
  df$WQ27_conc <- setWaterQuality(df$WQ27)
  df$WQ27 <- setWRiskBIN(df, WQ27_conc)#as.numeric()
  df$WS3 <- setWslocation(df$WS3)#as.numeric()
  df$WS7 <- setWaterSuff(df$WS7)#as.numeric()
  df$HH6 <- setArea(df$HH6)#as.numeric()
  df$WS1 <- setMainSource(df$WS1)
  df$WS2 <- setMainSource(df$WS2)
  df$WS4 <- setWs4time(df$WS4)
  df$WS8 <- setWhyInsuff(df$WS8)
  return(df)
}
